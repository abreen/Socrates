...
/**
 * An enumeration used to provide a lightweight representation of all known subclasses of Test. Each
 * enumeration value contains the string used in criteria files to refer to a test type, and type of
 * the file under which a test is specified. (This is necessary because we allow different file
 * types to specify a test with the same type.)
 *
 * This enumeration is used by SocratesConstructor, since it must map strings from the criteria file
 * to an actual class in the source code to instantiate. Then TestFactory does the instantiation.
 *
...
public enum TestType {
    ...
    public static TestType fromTypeAndString(FileType type, String testType) {
        if (testType.equals("script")) return SCRIPT;
        if (testType.equals("takesawhile")) return TAKES_AWHILE;

        for (TestType t : TestType.values())
            if (t.forFileType == type && t.type.equals(testType)) return t;
    ...
    public String toString() {
        String s = type + " test";
        if (forFileType != null) s += " for " + forFileType + " file";
        return s;
    }
}
